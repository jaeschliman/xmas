...
(defun add-star-bolt (sprites ship bolts)
  (let* ((bolt (make-instance 'bolt
    ...
    (run-action sub (hue-cycle-with-offset  cycle-len hue2 saturation) :repeat :forever)
    (run-action sub2 (rotate-by 0.5 -360.0) :repeat :forever)
    (run-action sub2 (hue-cycle-with-offset  cycle-len hue2 saturation) :repeat :forever)
    (run-action sub2 (list (move-by 0.3 10.0 10.0)
                           (move-by 0.3 -10.0 -10.0))
                :repeat :forever)
    (run-action bolt (hue-cycle-with-offset cycle-len hue saturation) :repeat :forever)
    (add-child bolt sub)
    (add-child bolt sub2)
    (vector-push-extend bolt bolts)
    (add-child sprites bolt)))
